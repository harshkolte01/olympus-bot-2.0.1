    ...
    async def _create(self, ctx, name, *, message):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute("SELECT COUNT(*) FROM autoresponses WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                ...
                if count >= 20:
                    return await ctx.reply(embed=discord.Embed(title="<a:max_cross2:1346031247192883254> Error!",
            ...
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<a:emoji_1740993086003:1346047306230792204> Success",
    ...
    async def _config(self, ctx):
        ...
        if not autoresponses:
            return await ctx.reply(embed=discord.Embed(
                description=f"<a:max_cross2:1346031247192883254> | There are no autoresponders in {ctx.guild.name}",
                color=0x00FFFF
            ))
