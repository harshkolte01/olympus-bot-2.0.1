...
class Automod(commands.Cog):
    ...
    async def punishment(self, ctx):
        ...
        embed = discord.Embed(title=f"Current Automod Punishments for {ctx.guild.name}", color=0x00FFFF)
        punishment_map = {}
        ...
        async def select_callback(interaction):
            ...
            for punishment in ["Mute", "Kick", "Ban"]:
                ...
                async def punishment_callback(button_interaction, selected_events=selected_events, punishment=punishment):
                    ...
                    updated_embed = discord.Embed(title=f"Updated Automod Punishments for {ctx.guild.name}", color=0x00FFFF)
    ...
    async def ignore_show(self, ctx):
        ...
        embed = discord.Embed(title="Ignored Channels & Roles for Automod", color=0x00FFFF)
    ...
    async def unignore_channel(self, ctx, channel: discord.TextChannel):
        ...
        if result.rowcount > 0:
            ...
            embed = discord.Embed(title="<:olympus_cross:1227866668152393789> Error", description=f"{channel.mention} is not in the automod ignore list.", color=0x00FFFF)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def unignore_role(self, ctx, role: discord.Role):
        ...
        if result.rowcount > 0:
            ...
            embed = discord.Embed(title="<:olympus_cross:1227866668152393789> Error", description=f"{role.mention} is not in the automod ignore list.", color=0x00FFFF)
            embed.set_footer(text=f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
            await ctx.send(embed=embed)
    ...
    async def config(self, ctx):
        ...
        current_punishments = await self.get_current_punishments(guild_id)
        embed = discord.Embed(title=f"Enabled Automod Events & their punishment type for {ctx.guild.name}", color=0x00FFFF)
