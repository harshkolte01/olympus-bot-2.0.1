    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.ctx.author:
            await interaction.response.send_message(
                embed=discord.Embed(description="Sorry only the requested author can control this", color=0x00FFFF),
                ephemeral=True
            )
            return False
        return True
...
class Map(commands.Cog):
    ...
    async def map(self, ctx, *, location: str):
        ...
        if view.coordinates == (None, None):
            ...
            return
        embed = discord.Embed(title=f"<:mapa:1257656719639707689> Map of {location}", color=0x00FFFF)
