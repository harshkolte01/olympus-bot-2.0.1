...
async def do_removal(ctx, limit, predicate, *, before=None, after=None):
  ...
  deleted = len(deleted)
  messages = [f'<a:emoji_1740993086003:1346047306230792204> | {deleted} message{" was" if deleted == 1 else "s were"} removed.']
  if deleted:
      messages.append("")
  ...
  if len(to_send) > 2000:
      await ctx.send(f"<a:emoji_1740993086003:1346047306230792204> | Successfully removed {deleted} messages.", delete_after=3)
  else:
...
class Owner(commands.Cog):
    ...
    async def staff_add(self, ctx, user: discord.User):
        if user.id in self.staff:
            sonu = discord.Embed(title="<:olympus_notify:1227866804630720565> Access Denied", description=f"{user} is already in the staff list.", color=0x00FFFF)
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
            sonu2 = discord.Embed(title="<a:emoji_1740993086003:1346047306230792204> Success", description=f"Added {user} to the staff list.", color=0x00FFFF)
    ...
    async def staff_remove(self, ctx, user: discord.User):
        if user.id not in self.staff:
            sonu = discord.Embed(title="<:olympus_notify:1227866804630720565> Access Denied", description=f"{user} is not in the staff list.", color=0x00FFFF)
            await ctx.reply(embed=sonu, mention_author=False)
        else:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
                sonu2 = discord.Embed(title="<a:emoji_1740993086003:1346047306230792204> Success", description=f"Removed {user} from the staff list.", color=0x00FFFF)
            await ctx.reply(embed=sonu2, mention_author=False)
    ...
    async def dm(self, ctx, user: discord.User, *, message: str):
        ...
        try:
            ...
            await ctx.send(f"<a:emoji_1740993086003:1346047306230792204> | Successfully Sent a DM to **{user}**")
        except discord.Forbidden:
    ...
    async def change_nickname(self, ctx, *, name: str = None):
        ...
        try:
            ...
            if name:
                await ctx.send(f"<a:emoji_1740993086003:1346047306230792204> | Successfully changed nickname to **{name}**")
            else:
                await ctx.send("<a:emoji_1740993086003:1346047306230792204> | Successfully removed nickname")
        ...
        if member:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"<a:emoji_1740993086003:1346047306230792204> | **{member.name}** has been successfully banned from {ctx.guild.name} by the Bot Owner.",
                    color=0x00FFFF)
        ...
        if user:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"<a:emoji_1740993086003:1346047306230792204> | **{user.name}** has been successfully unbanned from {ctx.guild.name} by the Bot Owner.",
                    color=0x00FFFF
                )
    ...
    async def bdg(self, ctx):
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(description='Invalid `bdg` command passed. Use `add` or `remove`.', color=0x00FFFF)
            await ctx.send(embed=embed)
    ...
    async def add(self, ctx, member: discord.Member, badge: str):
        ...
        if badge in BADGE_URLS or badge == 'bug' or badge == 'all':
            if badge == 'all':
                ...
                embed = discord.Embed(description=f"All badges added to {member.mention}.", color=0x00FFFF)
                await ctx.send(embed=embed)
            else:
                success = add_badge(user_id, badge)
                if success:
                    embed = discord.Embed(description=f"Badge `{badge}` added to {member.mention}.", color=0x00FFFF)
                else:
                    embed = discord.Embed(description=f"{member.mention} already has the badge `{badge}`.", color=0x00FFFF)
                await ctx.send(embed=embed)
        else:
            embed = discord.Embed(description=f"Invalid badge: `{badge}`", color=0x00FFFF)
            await ctx.send(embed=embed)
    ...
    async def remove(self, ctx, member: discord.Member, badge: str):
        ...
        if badge in BADGE_URLS or badge == 'bug' or badge == 'all':
            if badge == 'all':
                ...
                embed = discord.Embed(description=f"All badges removed from {member.mention}.", color=0x00FFFF)
                await ctx.send(embed=embed)
            else:
                success = remove_badge(user_id, badge)
                if success:
                    embed = discord.Embed(description=f"Badge `{badge}` removed from {member.mention}.", color=0x00FFFF)
                else:
                    embed = discord.Embed(description=f"{member.mention} does not have the badge `{badge}`.", color=0x00FFFF)
                await ctx.send(embed=embed)
        else:
            embed = discord.Embed(description=f"Invalid badge: `{badge}`", color=0x00FFFF)
            await ctx.send(embed=embed)
...
class Badges(commands.Cog):
    ...
    async def badges(self, ctx, member: discord.Member = None):
        ...
        if has_badges:
            ...
            embed = discord.Embed(title=f"{member.display_name}'s Profile", color=0x00FFFF)
